...
public sealed class JsonDataStore
{
    ...
    public (bool Success, string Error, AttendanceResponse? Record) AddAttendance(CreateAttendanceRequest request)
    {
        ...
        lock (_fileLock)
        {
            ...
            var attendance = new AttendanceEntity
            {
                ...
                Status = GetAttendanceStatus(request.Date, request.TimeIn),
                CreatedAtUtc = DateTime.UtcNow.ToString("O")
            };
    ...
    // Mark Present when time-in is still in the future or within 15 minutes from now.
    // Mark Late when the current time is more than 15 minutes after the time-in.
    private static string GetAttendanceStatus(string dateText, string timeInText)
    {
        if (!TryParseAttendanceDateTime(dateText, timeInText, out var attendanceDateTime))
        {
            return "Present";
        }
        ...
        if (attendanceDateTime >= now)
        {
            return "Present";
        }

        var lateMinutes = (now - attendanceDateTime).TotalMinutes;
        return lateMinutes > 15 ? "Late" : "Present";
    }

    // Parse combined attendance date + time from common frontend formats.
    private static bool TryParseAttendanceDateTime(string dateText, string timeInText, out DateTime attendanceDateTime)
    {
        attendanceDateTime = default;

        var cleanDateText = (dateText ?? string.Empty).Trim();
        var cleanTimeText = (timeInText ?? string.Empty).Trim();

        if (string.IsNullOrWhiteSpace(cleanDateText) || string.IsNullOrWhiteSpace(cleanTimeText))
        {
            return false;
        }

        var combinedText = $"{cleanDateText} {cleanTimeText}";

        if (DateTime.TryParse(combinedText, CultureInfo.InvariantCulture, DateTimeStyles.None, out var invariantParsed))
        {
            attendanceDateTime = invariantParsed;
            return true;
        }

        if (DateTime.TryParse(combinedText, out var localParsed))
        {
            attendanceDateTime = localParsed;
            return true;
        }
